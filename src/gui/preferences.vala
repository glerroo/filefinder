...
//         set_titlebar (hb);
        ...
        // General preferences - Excluded locations
        Adw.PreferencesPage general_page = new Adw.PreferencesPage ();
        general_page.set_title ("General");
        // general_page.set_icon_name ("");
        this.add (general_page);

        Adw.PreferencesGroup group = new Adw.PreferencesGroup ();
        general_page.add (group);

        Adw.SwitchRow cb_mounts = new Adw.SwitchRow ();
        ...
        cb_links.activated.connect (()=>{
            check_links = cb_links.active;
            is_changed = true;
        });

        cb_hidden.activated.connect (()=>{
            check_hidden = cb_hidden.active;
            is_changed = true;
        });

        cb_backup.activated.connect (()=>{
            check_backup = cb_backup.active;
            is_changed = true;
        });

        Adw.PreferencesGroup group2 = new Adw.PreferencesGroup ();
        group2.set_title ("User defined excluded locations");
        general_page.add (group2);

        var  view_excluded = new ExcludedLocationListBox ();
        group2.add(view_excluded);
...
//         vbox.add (button);
        ...
        //UI Settings
        Adw.PreferencesPage ui_settings_page = new Adw.PreferencesPage ();
        ui_settings_page.set_title ("Interface");
        // ui_settings_page.set_icon_name ("");
        Adw.PreferencesGroup group_a = new Adw.PreferencesGroup ();
        ui_settings_page.add (group_a);
        this.add (ui_settings_page);

        Adw.SwitchRow cb_vertical = new Adw.SwitchRow ();
        cb_vertical.set_title ("Split Vertical");
        ...
        cb_dark.activated.connect (()=>{
            Gtk.Settings.get_default().set ("gtk-application-prefer-dark-theme", cb_dark.active);
            is_changed = true;
        });

        // spin_rows.input.connect ((out new_value)=>{
            // if (Filefinder.window == null) return;
            // Filefinder.window.set_max_filters ((int)spin_rows.get_value ());
        //     is_changed = true;
        // });
        cb_single.activated.connect (()=>{
        ...
        // TODO: next
        Adw.PreferencesGroup group_a1 = new Adw.PreferencesGroup ();
        group_a1.set_title ("Choose the information to appear in the result view.");
        ui_settings_page.add (group_a1);
        this.add (ui_settings_page);

        Adw.SwitchRow view_name = new Adw.SwitchRow ();
...
//         scroll.add (view);
        ...
        //MIME Groups
        Adw.PreferencesPage mime_groups_page = new Adw.PreferencesPage ();
